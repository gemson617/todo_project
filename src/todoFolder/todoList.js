...
import React ,{ useState,useEffect,useRef,useReducer } from 'react';
    ...
    export default function TodoList({todo,dispatchTodo}){
        ...
        return(
            <div className={`p-2 rounded-lg hover:bg-slate-800 ${isClicked ? 'animate-wave' : ''}`} onClick={handleDivClick}>
                <span className="flex justify-between gap-2 border-b-2 border-b-white hover:border-b-slate-300"> 
                    <div className="flex items-center gap-2">
                        <input className="w-5 h-5 border-gray-300 rounded-xl focus:ring-indigo-500 checked:bg-indigo-500 checked:border-transparent" type="checkbox" checked={todo.complete} onChange={() => dispatchTodo({ type: 'toggle', payload: { id: todo.id } })} />
                        <h2 className={`${todoColor} ml-2`}>{todo.name}</h2>
                    </div>
                    <button type='button' className="flex items-center" onClick={() => dispatchTodo({ type: 'remove', payload: { id: todo.id } })}>
